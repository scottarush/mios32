...
s32 KEYBOARD_Init(u32 mode) {
	...
	for (kb = 0; kb < KEYBOARD_NUM; ++kb, ++kc) {
		...
		kc->two_split_middle_note_number = 60;   // C3 - Middle C
		kc->two_split_right_shift = -36;
		kc->three_split_left_note_number = 42;   // F1
		kc->three_split_left_shift = 24;
		kc->three_split_right_note_number = 85;  // C5
...
#if !KEYBOARD_DONT_USE_MIDI_CFG
	if (kc->midi_chn) {
		// Adjust the midi channels and note shifts for split mode
		...
		if (kc->split_mode == KEYBOARD_SPLIT_MODE_TWO){
			...
			if (note_number >= kc->three_split_right_note_number){
				...
				// Note is in zone 1 so add the offset.
		...
		for (i = 0; i < 16; ++i, mask <<= 1) {
			if (kc->midi_ports & mask) {
				...
				if (ctrl_number < 128){
					...
					// If split mode is on, then also send the same cc on the other midi channels
					switch(kc->split_mode){
						case KEYBOARD_SPLIT_MODE_TWO:
						MIOS32_MIDI_SendCC(port, midi_chn, ctrl_number, value);
						break;
						case KEYBOARD_SPLIT_MODE_THREE:
						MIOS32_MIDI_SendCC(port, midi_chn, ctrl_number, value);
						MIOS32_MIDI_SendCC(port, midi_chn+1, ctrl_number, value);
						break;

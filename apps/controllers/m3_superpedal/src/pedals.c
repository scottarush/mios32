...
void PEDALS_NotifyChange(u8 pedalNum,u8 value,u32 timestamp){
    ...
    if ((pedalNum == 0) || (pedalNum > pc->num_pedals+1)){
        DEBUG_MSG("Invalid pedalNum");
    }
    if (pedalNum == pc->num_pedals+1){
        // This is the make switch. check for pressed. 
        if (value) {
            // just clear flag and return
            makePressed = 0;
            return;
        }   
        ...
        if (pendingPedalNum != 0) {
            ...
            // Now compute velocity for press
            ...
            // Send note on for the pedal and clear pending
    ...
    // Otherwise, this is a pedal switch event.
    ...
    // Otherwise, this is a press.  If make already pressed, then send the note at the last sent 
    // velocity.
    if (makePressed){
    ...
    // 2. Wait for the make switch to compute velocity

...
//! Modified by Scott Rush to:
//  1. support ascending/descending arpeggiator which requires multiple copies of the same note in teh stack, and
//  2. Support a note length for each stack event.
//!
//! Usage Examples:
...
s32 NOTESTACK_Push(notestack_t* n, u8 new_note, u8 tag)
{
   ...
   if (hold_mode) {
      ...
      // Commented out by Scott Rush to support multiple copies of same note in stack
...
s32 NOTESTACK_SendDebugMessage(notestack_t* n)
{
   ...
   for (i = 0; i < n->len; ++i) {
      MIOS32_MIDI_SendDebugMessage("%02d: %s%-3d (0x%02x) %s tag:0x%02x length:%d\n",
         i,
